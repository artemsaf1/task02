...
Замечу, что про честь речи не было. Последняя просьба. Я смотрю, ты неплохо управляешься с гитом. В моей библиотеке не хватает некоторых мануалов, в частности по работе с GitHub. Я не люблю, когда не хватает информации. Создай небольшой и лаконичный мануал в разметке Markdown в файлике src/github_manual.md. Меня волнуют следующие вопросы, минимум 5 из которых хорошо бы подкрепить скриншотами: 
...
***== Получен Quest 4. Подготовить краткий мануал по пунктам выше по использованию GitLab в файле src/github_manual.md в разметке Markdown. Прикрепить по скриншоту минимум к 5 пунктам. ==***
